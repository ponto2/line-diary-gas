...
function sendWeeklyReview() {
  ...
  // 1-2. AIへのコンテキスト作成
  let reviewContext = "あなたはユーザーの専属メンターAIです。過去1週間の日記データを分析し、LINEで送るための週次レビューを作成してください。\n\n";
  
  reviewContext += "【⚠️ 重要な制約事項】\n";
  reviewContext += "1. **Markdownの太字記法（アスタリスク2つでの囲み）は絶対に使用禁止**です。LINEでは表示崩れの原因になります。\n";
  ...
  reviewContext += "   - 精神論ではなく、データに基づいた「来週こうするともっと良くなる」という具体的な行動を1つ提案してください。\n\n";
